...
public final class TokenAuthenticationFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		System.out.println("AUTHHDHDHDH " + authenticated);
		if(authenticated) {
		...
		if(!authenticated) {
			...
			if(canContinueWithRequest(httpRequest)) {
				System.out.println("continue auth");
				httpRequest.getSession().setAttribute("auth", "true");
			}
			System.out.println(" === AUTHENTICATION MY: " + SecurityContextHolder.getContext().getAuthentication());
		...
		if (canRequestProcessingContinue(httpRequest)) {
			System.out.println("Continuing with the request");
			chain.doFilter(request, response);
		}*/
	...
	private void checkLogin(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		if (authorization != null) {
			...
			//setCanContinueWithRequest(httpRequest, false);
	...
	private void checkUsernameAndPassword(String username, String password, HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		if (tokenInfo != null) {
			...
			setCanContinueWithRequest(httpRequest, true);
		} else {
	...
	private void checkLogout(HttpServletRequest httpRequest) {
		if (currentLink(httpRequest).equals(logoutLink)) {
			...
			//doNotContinueWithRequestProcessing(httpRequest);
			setCanContinueWithRequest(httpRequest, false);
			httpRequest.getSession().setAttribute("auth", null);
		}
	}
	...
	private boolean canContinueWithRequest(HttpServletRequest httpRequest) {
		return !(null == httpRequest.getAttribute("CONTINUE_WITH_REQUEST") || (Boolean)httpRequest.getAttribute("CONTINUE_WITH_REQUEST") == false);
	}
	
	private void setCanContinueWithRequest(HttpServletRequest httpRequest, boolean canContinue) {
		httpRequest.setAttribute("CONTINUE_WITH_REQUEST", canContinue);
	}
}
